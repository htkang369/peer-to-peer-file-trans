...
public class Peer {
	...
	public static synchronized void selectPreferredNeighbors() {
			...
			if (unChokedMap.get(unChokePeerID) == null) {
				if (optimisticNeighbor != null) {
					if (optimisticNeighbor.peerID != unChokePeerID) {
						...
						System.out.println("unChokedMap add new" + unChokePeerID + " speed = "
								+ unChokedMap.get(unChokePeerID).speed);
						sendUnchoke(unChokedMap.get(unChokePeerID));
						chokedMap.remove(unChokePeerID);
					} else {
						System.out.println(
								unChokePeerID + "op do not need send unchoke when it is select as preferred too");
					}
				} else {
					unChokedMap.put(unChokePeerID, interestedList.get(i));
					System.out.println("op is null, and unChokedMap add new" + unChokePeerID + " speed = "
							+ unChokedMap.get(unChokePeerID).speed);
					sendUnchoke(unChokedMap.get(unChokePeerID));
					chokedMap.remove(unChokePeerID);
				}
		...
		System.out.println(
				"select preferredNeighbors interestedList.size = " + interestedList.size() + " Config.k :" + Config.k);
			...
			if (chokedMap.get(chokePeerID) == null) {
				if (optimisticNeighbor != null) {
					if (chokePeerID != optimisticNeighbor.peerID) {
						...
						System.out.println("chokedMap add new" + chokePeerID + "interestedList peerID"
								+ interestedList.get(i).peerID);
					...
					System.out.println("optimisticNeighbor is null choke Peer ID: " + chokePeerID
							+ " interestedList peerID" + interestedList.get(i).peerID);
					chokedMap.put(chokePeerID, interestedList.get(i));
	...
	public static synchronized void selectOptimisticallyUnchokedNeigbor() {
		...
		if (size > 0) {
			...
			if (optimisticNeighbor != null) {
				...
				MyUtil.time();
				System.out.println("Peer [" + myID + "] has the optimistically unchoked neighbor ["
	...
	public static void tryToConnect() throws UnknownHostException, IOException {
		...
		while (iter.hasNext()) {
			...
			try {
				...
				System.err.println(
						"Connection refused. You need to initiate a server first." + Config.peerIpAddress.get(temp));
				requestSocket.close();
			}
		}
	...
	public static class Handler extends Thread implements Comparable<Handler> {
		...
		public void sendBitfield() {
			System.out
					.println("send Bitfield Message bitFieldLength = " + Config.bitFieldLength + " peerID: " + peerID);
		...
		public synchronized void receiveBitfield() throws Exception {
			...
			System.out.println("parse Bitfield Message from:" + peerID + " msgLength = " + msgLength
					+ " bitfield length =" + Config.bitFieldLength);
			MyUtil.pw.println("parse Bitfield Message from:" + peerID + " msgLength = " + msgLength
					+ " bitfield length =" + Config.bitFieldLength);
			MyUtil.pw.flush();
		 ...
		 * 
		 * For example, suppose that peer A makes a connection to peer B and receives a
		 * 'bitfield' message to peer B. In another example, suppose that peer A
		 * receives a 'have' message from peer C that contains the index of a piece not
		 * in peer A. Then peer A sends an 'interested' message to peer C.
		 * 
		...
		public void sendPieceMsg(int pieceNum) {
			...
			if(!checkPeerBitfieldWhetherHave(pieceNum)){
		...
		public boolean checkPeerBitfieldWhetherHave(int pieceNum) {
			int index = pieceNum / 8;
			int offset = pieceNum % 8;
			int temp = 0x01 << (7 - offset);
			...
			System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!receive a request " + pieceNum + " peer already have from " + peerID);
			return true;
		}
		...
		public void sendHave(int pieceIndex) {
			...
			MyUtil.time();
			System.out.println("Peer [" + peerID + "] received the 'have' message from [" + myID + "] for the piece ["
					+ pieceIndex + "]");
		...
		/**
		 * if A receives a bitfield message form B, finds out whether B has pieces that
		 * it doesn't have
		 */
...
//				System.out.println("localBitfield.bitfield:" + localBitfield.bitfield[i] + " peerID: "+peerID);
				...
				System.out.println(
						" i = " + i + " Config.bitFieldLength  " + Config.bitFieldLength + " peerID: " + peerID);
				...
				if (temp != 0) {
					...
					for (int j = 0; j < 8; j++) {
						...
						if (k == 1) {
							...
							System.out.println(
									"find out interested piece, pieceNum = " + piecenum + " peerID: " + peerID);
						}
					}
				}
			}
...
//						System.arraycopy(oneByte, 0, rawMsg, count, 1);
				...
				case ActualMsg.PIECE:
					...
					if (!checkBitfieldWhetherHave(piecenum)) {
						...
						System.out.println("Now the number of pieces it has is ["
		...
		public synchronized boolean checkFileComplete() {
			for (int i = 0; i < Config.bitFieldLength - 1; i++) {
				System.out.println("checkFileComplete  localBitfield.bitfield[" + i + "]"
						+ String.format("%02X", localBitfield.bitfield[i]) + " peerID: " + peerID);
				byte not = (byte) ~localBitfield.bitfield[i];
				...
				if (temp != 0) {
					System.out.println(
							"checkFileComplete  temp " + String.format("%02X", temp) + "!=0 peerID: " + peerID);
					fileComplete = false;
					return false;
				}
			}
		...
		public void sendRequestMsg() {
			...
			if (length_interest > 0) {
				...
				System.out.println(
						"send Request Message" + " peerID: " + peerID + " piecenum = " + MyUtil.byteArrayToInt(a));
				sendMessage(c);
				a = null;
				c = null;
			} else {
		...
		public synchronized void changeLocalBitField(int piecenum) {
			...
			int offset = piecenum % 8;
			int temp = 0x01 << (7 - offset);
			System.out.println("change local bitfield piecenum =  " + piecenum + " originl:"
					+ String.format("%02X", localBitfield.bitfield[index]) + " index = " + index + " offset: "
					+ offset);
			localBitfield.bitfield[index] = (byte) (localBitfield.bitfield[index] | temp);
			System.out.println("change local bitfield piecenum =  " + piecenum + " new:"
					+ String.format("%02X", localBitfield.bitfield[index]) + " index = " + index + " offset: "
					+ offset);
		...
		public synchronized void changePeerBitField(int piecenum) {
			...
			if ((not & temp) != 0) {
				// localBitfield.bitfield[index] = (byte) (localBitfield.bitfield[index] |
				// temp);
				System.out.println("receive new interested have from " + peerID + " pieceNum = " + piecenum
						+ " peerID: " + peerID);
				isInterested = true;
			...
			System.out.println(
					"change peerBitfield piecenum =  " + piecenum + " index = " + index + " offset: " + offset);
		...
		void computeDownloadRate() {
			...
			System.out.println("peerID: " + peerID + " update speed =" + speed + " downloadThroughput "
					+ downloadThroughput + " upLoadThroughput" + upLoadThroughput + " startTime " + startTime + " now"
					+ timeInMillis);
		}
